class Transform
  ...
  def cut(items)
    items = items.map! { |item| item.downcase.strip.split(',') }
    items.each{|item| item.each {|i| i.strip!}}
  end
  ...
  def classify_good(item)
    !@exclusions.any? {|a| item[1].include?(a)}
  end

  def classify_import(item)
    check_import = item[1].include? 'imported'
